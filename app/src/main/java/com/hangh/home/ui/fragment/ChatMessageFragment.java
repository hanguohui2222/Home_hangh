package com.hangh.home.ui.fragment;
...
import com.hangh.home.ui.activity.MainActivity;
import com.hangh.home.ui.activity.SessionActivity;
import com.hangh.home.factory.ThreadPoolFactory;
import com.hangh.home.imageloader.ImageLoaderManager;
import com.hangh.home.bean.Contact;
import com.hangh.home.nimsdk.NimFriendSDK;
import com.hangh.home.nimsdk.NimRecentContactSDK;
...
public class ChatMessageFragment extends BaseFragment {
    ...
    public View initView() {
        View view = View.inflate(getActivity(), R.layout.frag_chat_message, null);
...
//        if (mAdapter == null) {
        mAdapter = new LQRAdapterForRecyclerView<RecentContact>(getActivity(), R.layout.item_message_rv,
                mRecentContactList) {
            @Override
                        ...
                        public void run() {
                            ...
                            UIUtils.postTaskSafely(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    if (team.isMyTeam()) {
                                        if (team != null)
                                            helper.setText(R.id.tvName, TextUtils.isEmpty(team.getName()) ? "群聊(" +
                                                    team.getMemberCount() + ")" : team.getName());
                                    } else {

                                        NimRecentContactSDK.deleteRecentContact(item);
                                        //TODO  java.lang.ArrayIndexOutOfBoundsException: length=10; index=-1
...
//                    MoonUtil.identifyFaceExpression(getActivity(), helper.getView(R.id.tvMsg), item.getContent(),
// ImageSpan.ALIGN_BOTTOM);
                MoonUtils.identifyFaceExpressionAndTags(getActivity(), helper.getView(R.id.tvMsg), item.getContent(),
                        ImageSpan.ALIGN_BOTTOM, 0.45f);

                //判断是否有未读消息
                helper.setViewVisibility(R.id.tvUnread, item.getUnreadCount() > 0 ? View.VISIBLE : View.GONE).setText
                        (R.id.tvUnread, String.valueOf(item.getUnreadCount()));
...
//        }
            ...
            public void onResult(int code, List<RecentContact> result, Throwable exception) {
                ...
                for (int i = 0; i < result.size(); i++) {
                    ...
                    if (rc.getSessionType() == SessionTypeEnum.Team) {
                        if (NimTeamSDK.queryTeamBlock(rc.getContactId()) != null
                                &&!NimTeamSDK.queryTeamBlock(rc.getContactId()).isMyTeam()) {
                            result.remove(i);
    ...
    private void updateTotalUnReadCount() {
        ...
        if (activity.tvMessageCount != null)
            if (totalUnreadCount > 0) {
                activity.tvMessageCount.setVisibility(View.VISIBLE);
                activity.tvMessageCount.setText(String.valueOf(totalUnreadCount > 99 ? 99 : totalUnreadCount));
            } else {
                activity.tvMessageCount.setVisibility(View.GONE);
            }
    }
